...
axo_goi_map <- final_bridge |>
  filter(toupper(amex_symbol) %in% GOI,
         amex_id %in% rownames(axo_annotated)) |>
  mutate(gene = toupper(amex_symbol)) |>
  select(amex_id, gene) |>
  distinct()
...
# Create a renamed axolotl object for plotting (display symbols not AMEX IDs)
axo_plot <- axo_annotated
mat_orig <- get_data_sparse(axo_plot)

# Build a renamed assay with gene-symbol rownames for dotplot legibility
...
axo_goi_symbols <- unique(axo_goi_map$gene)
axo_goi_present <- intersect(axo_goi_symbols, rownames(axo_plot[["RNA_named"]]))
